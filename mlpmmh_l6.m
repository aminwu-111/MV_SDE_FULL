...
Nl(1) = floor(0.2 *2^(2*LP - 6/7*(Lmin)) +2000);
ML(1) = floor(0.2 *2^(2*LP - 0.5*(Lmin)) +120);
X0_m{1,1} = zeros(d,ML(1));
X0_m{1,1} = sample_X0(params,ML(1));
for l = 2:LP - Lmin+1
    Nl(l) = floor(0.2 *2^(2*LP - 6/7*(l+Lmin-1)) +500);
    ML(l) = floor(0.2 *2^(2*LP - 0.5*(l+Lmin-1)) +120);
    X0_m{l,1} = zeros(d,ML(l));
	X0_m{l,1} = sample_X0(params,ML(l));
end
%Nl = [1,1000];
...
X_measure = simulate_law_model(delta, T, X0_m{1,1}, Theta_SIG_p(1),Theta_SIG_p(2),Theta_SIG_p(3),Theta_SIG_p(4),Theta_SIG_p(5),Theta_SIG_p(6));
...
for iter = 1:Nl(1)
    ...
    X_measure_prime = simulate_law_model(delta, T, X0_m{1,1}, Theta_SIG_prime(1),Theta_SIG_prime(2),Theta_SIG_prime(3),Theta_SIG_prime(4),Theta_SIG_prime(5),Theta_SIG_prime(6));
...
for l = 1:LP - Lmin 
   ...
   Theta_SIG_l = [Theta_l(1), Theta_l(2), exp(Theta_l(3)), exp(Theta_l(4)),exp(Theta_l(5)),exp(Theta_l(6)),exp(Theta_l(7)),exp(Theta_l(8)),exp(Theta_l(9))];
    ...
    [X_1_measure, X_2_measure] = simulate_coupled_law(delta_t, T, X0_m{l+1}, X0_m{l+1}, Theta_SIG_l(1), Theta_SIG_l(2),Theta_SIG_l(3),Theta_SIG_l(4),Theta_SIG_l(5),Theta_SIG_l(6));
    ...
    for iter = 1:Nl(l+1)
        ...
        [X_1_measure_p, X_2_measure_p] = simulate_coupled_law(delta_t, T, X0_m{l+1}, X0_m{l+1}, Theta_l_SIG_prime(1),Theta_l_SIG_prime(2),Theta_l_SIG_prime(3),Theta_l_SIG_prime(4),Theta_l_SIG_prime(5),Theta_l_SIG_prime(6));
...
figure
plot(burnin:Nl(l+1), Theta_traceN{l+1,1}(burnin:end,8), 'r--')
title('log(\tau_2)')
hold off

figure
plot(burnin:Nl(l+1), Theta_traceN{l+1,1}(burnin:end,9), 'r--')
title('log(\tau_3)')
hold off
end
%}
...
function [lwf,lwc,cz] = particle_filter_coupled(Y, X_1_measure, X_2_measure, delta_t, T, X0_1, X0_2, I, J, c, lambda, b_ext, Gamma, sigma_obs_1, sigma_obs_2, sigma_obs_3)
    ...
    for i = 1:steps_count_1
        ...
        if mod(i, 1/delta_t)==0
            ...
            log_w_1 = log_normpdf(X_1(:,:,i+1), Y(i*delta_t), diag([sigma_obs_1, sigma_obs_2,sigma_obs_3]).^2);
            log_w_2 = log_normpdf(X_2(:,:,j+1), Y(i*delta_t), diag([sigma_obs_1, sigma_obs_2,sigma_obs_3]).^2);
